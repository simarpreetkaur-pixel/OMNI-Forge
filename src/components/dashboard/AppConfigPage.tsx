...
function getStepResponse(completedStep: 0, config: GenericConfig): string {
  return `Got it — **${config.useCase}**.\n\nStep 2 of 2: Who is the **target audience** or end user?\n\nExamples: New customers, Enterprise clients, Internal teams, Sales reps, Support agents`;
}
...
export default function AppConfigPage({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      const greeting = isAlreadySaved
        ...
        : `Hi! Let's get your **${app.name}** app set up — just **2 quick questions**.\n\nStep 1 of 2: What is the **primary goal or use case** for this app?\n\nExamples: Customer support, Lead qualification, After-sales service, Internal operations, Compliance tracking`;
  ...
  function handleSend() {
    ...
    setTimeout(() => {
      if (nextStep <= 1) {
        const response = getStepResponse(currentStep as 0, updatedConfig);
        setMessages((prev) => [...prev, { role: "ai", text: response }]);
        setFlowStep(nextStep as FlowStep);
        setIsTyping(false);
        inputRef.current?.focus();
      } else {
        // Both questions answered → building sequence
        const completionMsg = `All done — **${updatedConfig.targetAudience}**.\n\nSetting up your **${app.name}** app now…`;
        setMessages((prev) => [...prev, { role: "ai", text: completionMsg }]);

        const buildingSteps = [
          "Saving configuration...",
          `Provisioning **${app.name}** runtime...`,
  ...
  const stepNum = flowStep === "done" ? 2 : (flowStep as number);
      ...
      <div className="flex shrink-0 items-end border-b border-[#e7e7f0] bg-white px-8">
        ...
        {isGuided && activeTab === "ai" && (
          <div className="ml-auto flex items-center gap-2 pb-1 pr-1">
            <span className="text-[11px] text-[#a0a0a0]">Step {stepNum + 1} of 2</span>
              ...
              {skillReady ? (
                <div className="overflow-hidden rounded-xl border border-[#e7e7f0] bg-[#0e0e0e]">
                  <div className="flex items-center gap-2 border-b border-white/10 px-4 py-3">
                    ...
                    <span className="ml-2 text-[11px] text-white/40">
                      {app.name.toLowerCase().replace(/\s+/g, "-")}.skill.md
                    </span>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-[#e7e7f0] bg-white py-20 text-center">
                  ...
                  <p className="max-w-[280px] text-[12px] leading-[1.5] text-[#737373]">
                    Complete the AI assistant setup or fill in the Use case in the Configuration tab to generate your skill file.
                  </p>
                </div>
              )}
            </div>
